...
import jp.ac.hal.tokyo.cenka.beans.CreditAmountBean;
import jp.ac.hal.tokyo.cenka.beans.InventoryBean;
import jp.ac.hal.tokyo.cenka.beans.ProductsBean;


public class CreditAmountDao {
	private Connection con;

	public CreditAmountDao() throws NamingException, SQLException {
	...
	public CreditAmountDao(Connection con){
		this.con = con;
	}
	 ...
	 * @throws SQLException
	 */
	public List<CreditAmountBean> findAll() throws SQLException {

		PreparedStatement select = con.prepareStatement("select * from t_creditamount");

		ResultSet result = select.executeQuery();

		ArrayList<CreditAmountBean> table = new ArrayList<CreditAmountBean>();
		while (result.next()) {

			CreditAmountBean record = new CreditAmountBean();
			record.setConpany_id(result.getString("f_inventory_id"));
			record.setSredid_price(result.getInt("f_invent_quantity"));
			record.setUnpaid_price(result.getInt("f_schedule_quantity"));
			table.add(record);
		}
		return table;
	}
